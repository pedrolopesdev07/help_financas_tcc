...
export function Transactions({ transactions, onAdd, onDelete, initialOpenForm }: TransactionsProps) {
              ...
              <div key={t.id} className="bg-card rounded-2xl border border-border px-4 py-3 flex items-center gap-3 hover:border-primary/20 transition-all group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full">{c?.label}</span>
                    {t.recurrence !== "unica" && (
          ...
          <div className="bg-card rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 p-1 bg-muted rounded-xl">
                ...
                <button
                  type="button"
                  onClick={() => setForm(f => ({ ...f, type: "despesa", category: "alimentacao" }))}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    {categories
                      .filter(c => form.type === "receita" ? ["salario", "freelance", "investimento", "outros"].includes(c.id) : !["salario", "freelance", "investimento"].includes(c.id))
                      .map(c => (
                        <option key={c.id} value={c.id}>{c.emoji} {c.label}</option>
                      ))}
                  </select>
